...
#create jobs
with open("G:\\MAC_Research_Data\\Pan_cancer\\test.sam") as f:
    nextLineByte = f.tell() ## tell you what is the position of the line to start to be processed
    
    pool.map(process_wrapper, f)
        #jobs.append( pool.apply_async(process_wrapper,(nextLineByte)) )
        #nextLineByte = line.tell()
        #print(nextLineByte)
...
if __name__ == "__main__":
    pool = Pool(1)
    with open('G:\\MAC_Research_Data\\Pan_cancer\\test.sam') as source_file:
